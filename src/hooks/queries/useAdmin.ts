...
export const useApproveSubmission = () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const updatedSubmittedBounties = submittedBounties.filter(
        (id: string) => id !== bountyId,
      );
      ...
      const updatePromises =
        userList?.map(async (user: User) => {
          ...
          const updatedActiveBounties = activeBounties.filter(
            (item: string | Bounty) => {
          ...
          if (
            updatedActiveBounties.length !== activeBounties.length ||
      ...
      const submissionList = otherSubmissions.records as
        | Submission[]
        | undefined;
    ...
    queryFn: async () => {
      ...
      try {
        ...
        if (!response.records || response.records.length === 0) {
          ...
          const allSubmissionsList = allSubmissions.records as
            | Submission[]
            | undefined;
          const filtered =
            allSubmissionsList?.filter((submission: Submission) => {
        ...
        console.error(
          'Error fetching submissions for bounty',
          bountyId,
          ':',
          error,
        );
